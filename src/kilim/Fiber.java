...
public final class Fiber {
     ...
     * that will be needed when the Fiber is resumed. It is always kept equal to
     * stateStack[iStack] if iStack is in the (0..stateStack.length-1) range,
     * and null otherwise. This is used by the generated code to avoid having to
     * manipulate stateStack in the generated code, and to isolate all stack
     * manipulations to up() and down().
     */
     ...
     * the nth pausable method (after restoring the appropriate state). Accessed
     * by generated code (hence public).
     */
    public int pc;
    ...
    /*
     * Pausing, and have saved state from an earlier invocation, so nothing left
     * to do.
     */
    ...
    /*
     * The user calls pause(), but the weaver changes the call to pause(Fiber),
     * which alternates between pausing and not pausing in successive
     * invocations.
     * 
     * @param f
     */
     ...
     * 
     * If it is a normal return, it needs to read the state into curState and
     * nullify it in stateStack. This accomplishes the equivalent of: move up
     * get state() restore from state delete state()
     * 
    ...
    public int upEx() {
        // compute new iStack.
        int is = task.getStackDepth() - 2; // remove upEx and convert to 0-based
                                           // index.
        if (is >= stateStack.length)
            ensureSize(is + 1);
